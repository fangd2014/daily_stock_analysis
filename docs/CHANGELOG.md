...
- 📡 **PyTDX 分钟行情数据底座**
  ...
  - 提供月均 10% 收益、最大回撤 10% 的研究门槛配置；仅用于模拟验证，不构成收益保证
  - 新增 Tushare 历史分区与 PyTDX 近期行情混合加载，历史月份未补齐时禁止优化
  - 验收口径升级为至少 36 个样本外月份、月收益中位数 10% 和最大回撤 10%
...
- 🧺 **科技龙头双峰组合模拟盘**
  - 新增 20 只科技龙头候选池、市值/流动性门槛和无未来数据的双峰筛选器
  - 新增固定 3 只、100 万元总资金、30% 目标总仓位及按条件顺位补充机制
  - 首次选股和调股增加峰间位置不高于 45% 的可买入门槛，高位股票不得进入持仓池
  - 新增三个独立 T+1 模拟子账户、组合状态/周报汇总及可并行 cron 任务
