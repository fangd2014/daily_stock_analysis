...
### 新增
- 🖥️ **量化选股工作台**
  - 新增 `/quant` 独立页面，集中展示筹码双峰、热门板块中军、四因子时点选股和ETF动量轮动策略
  - 支持查看因子、买卖门槛、组合风控、脱敏参数、最近选股结果和模拟操作指引
  - 新增白名单策略手动启动、重复任务保护、异步状态查询和逐行进程日志展示
  - 统一归一化四套策略的报告字段，展示信号日期、参考价格、策略分、目标仓位、买入和止损参考
  - 执行记录和最多1000行日志原子写入 `reports/quant/web_runs/`，服务重启后仍可审计
  - 新增量化策略目录与运行API、凭据脱敏、命令白名单和任务生命周期回归测试
  - 建立根目录 `DESIGN.md`，明确金融终端视觉、响应式、可访问性和安全执行约束
- 🔄 **ETF动量轮动模拟盘**
  - 新增15只跨资产ETF池的20/60/120/250日风险调整动量评分、MA120绝对趋势门槛和资产类别分散约束
  - 新增100万元日频/周频事件回测，按下一交易日开盘成交并计入佣金、最低佣金、滑点和高开3%放弃条件
  - 新增单只固定/移动止损、组合回撤降仓、组合清仓冷静期和最多3只、60%总仓位风控
  - 使用Tushare基金复权因子消除ETF拆分污染，以TickDB交易日历确认下一交易日
  - 新增逐月收益、权益、成交、信号和下周计划报告，以及周日20:00飞书推送的幂等cron安装脚本
  - 修复组合回撤冷静期结束后未重置高水位、导致账户永久空仓的问题，并新增跨冷静期回归测试
  - 新增2021-07-26至2026-07-24五年前复权日线每日重算回测，结果为总收益-2.44%、最大回撤16.60%
  - 同一日频规则最近一年收益11.76%、最大回撤11.15%，长期五年失败结论优先于短期结果
  - 新增五年周频对照：总收益5.09%、年化1.00%、最大回撤14.06%，成交748笔、费用6927元
  - 周频相比日频降低约73%换手并改善收益，但仍未达到长期收益目标；最近一年17.83%不替代五年结论
- 📡 **TickDB 四因子时点选股**
  - 安装 TickDB Market Data Skill，并新增不持久化 API Key 的 REST 客户端和原子时点快照
  - `TICKDB_API_KEY`与`DEEPSEEK_API_KEY`统一只从项目`.env`读取，不再读取进程变量或Shell配置
  - 新增指数动量、舆情语义、分位区间学习/特征遮挡归因、真实竞价异动联动四个因子
  - 新增前夜观察名单与次日 9:25 竞价确认两阶段选股，最多 5 股且单行业最多 2 股
  - 缺少 A 股真实竞价盘口时禁止自动买入，不使用普通分钟 K 线伪造撤单和未匹配委托
  - 新增 60% 总仓位、单股 5% 止损、组合 12% 回撤停机及两档止盈风控
  - 新增可审计 CSV/JSON/Markdown 报告和因果性、凭据安全、行业限额回归测试
  - 新增全A收盘缓存实时运行器，按行业最多2只构建分散观察名单并用TickDB复核行情
- 📡 **PyTDX 分钟行情数据底座**
...
- 🌙 **每日筹码双峰10股筛选与AI指导**
  ...
  - 20:00 cron 启动器与手动任务统一由Python读取项目`.env`中的`DEEPSEEK_API_KEY`
- 🔥 **每日热门板块中军短线筛选**
